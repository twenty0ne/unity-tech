...
namespace PublicDomain.ZoneInfo
{
    ...
    public static class Database
    {
		...
		public static void LoadUnityResources(string folder)
		{
			UnityEngine.TextAsset[] txts = UnityEngine.Resources.LoadAll<UnityEngine.TextAsset>(folder);
			if (txts != null)
			{
				for (int i = 0; i < txts.Length; ++i)
				{
					LoadString(txts[i].text);
				}
			}
		}
