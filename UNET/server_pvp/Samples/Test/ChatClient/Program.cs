			...
			while ((im = s_client.ReadMessage()) != null)
			{
				...
				switch (im.MessageType)
				{
					...
					case NetIncomingMessageType.Data:
                        // int byteLen = (int)im.ReadVariableUInt32();
                        int byteLen = im.ReadInt32(); 
                        byte[] byteArray = im.ReadBytes(byteLen);

                        string chat = System.Text.Encoding.Default.GetString(byteArray);
                        // byte[] byteArray = im.readva
						Output(chat);
						break;
					default:
